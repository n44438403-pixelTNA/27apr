...
export const PdfView: React.FC<Props> = ({ 
  ...
  // 3D Models Support (Admin Toggle)
  useEffect(() => {
    if (settings?.enable3DModels) {
      ...
      let styleTag = document.getElementById('model-viewer-style');
      if (!styleTag) {
        styleTag = document.createElement('style');
        styleTag.id = 'model-viewer-style';
        document.head.appendChild(styleTag);
      }
      // Define default CSS for model-viewer to ensure it renders with a visible size
      // We also enforce environment lighting so the models don't appear black
      styleTag.innerHTML = `
        model-viewer {
          width: 100%;
          height: 300px;
      ...
      // Apply neutral environment to all existing and future model-viewer elements dynamically
      // This solves the issue where models appear entirely black due to missing lighting.
      const observer = new MutationObserver((mutations) => {
        mutations.forEach(mutation => {
          mutation.addedNodes.forEach(node => {
            if (node.nodeName === 'MODEL-VIEWER') {
              const mv = node as HTMLElement;
              if (!mv.hasAttribute('environment-image')) {
              ...
              if (!mv.hasAttribute('camera-controls')) {
               ...
               const viewers = element.querySelectorAll('model-viewer');
               viewers.forEach(mv => {
                 if (!mv.hasAttribute('environment-image')) {
                   mv.setAttribute('environment-image', 'neutral');
                 }
      ...
      (window as any).__modelViewerObserver = observer;
      ...
      styleTag.innerHTML = 'model-viewer { display: none !important; }';
    }

    // Cleanup not strictly necessary globally, but good for react lifecycle
    return () => {
        if ((window as any).__modelViewerObserver) {
            (window as any).__modelViewerObserver.disconnect();
            delete (window as any).__modelViewerObserver;
        }
        // We leave the script but ensure style applies if unmounted and disabled globally
    };
  }, [settings?.enable3DModels]);
